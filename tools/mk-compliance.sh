...
counter=0
  ...
  while IFS= read -r line; do
          ...
          if [ $counter -eq $total_lines ]; then
            ...
            printf '"%s": { "id": "%s", "name": "%s", "description": "%s", "type": "inventory", "condition_for": "passing", "rules": [ { "attribute": "SCC pass", "operator": "matches", "value": "%s" } ], "category": "%s", "severity": "%s", "host_filter": "default:scc_inventory_enabled" }\n' $ID $ID $ID "$DESCRIPTION" $ID "$CATEGORY" $SEVERITY >> "$compliance_filename"
          else
            ID=$(echo "$line" | awk '{print $1}')
            DESCRIPTION=$(echo "$line" | cut -d ' ' -f4- | sed 's|/|\/|g')
            DESCRIPTION=$(echo "$DESCRIPTION" | sed "s|\"|'|g")
